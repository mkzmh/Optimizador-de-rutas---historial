...
import pandas as pd  # <--- Necesitamos pandas para unir las capas
import momepy
...
# =============================================================================
# 2. FUNCIONES DE CARGA DE MAPA (CORREGIDO PARA MULTI-CAPA)
# =============================================================================

def cargar_mapa_kml():
    """
    Lee TODAS las capas del KML y las une en un solo grafo.
    """
    global GRAFO_GLOBAL
    if GRAFO_GLOBAL is not None: return GRAFO_GLOBAL

    try:
        # Habilitar soporte KML
        ...
        # 1. Identificar todas las capas (Layers) dentro del KML
        capas = fiona.listlayers(ARCHIVO_KML)
        print(f"📂 Capas encontradas en el mapa: {capas}")
        
        lista_gdfs = []
        
        # 2. Leer cada capa por separado
        for capa in capas:
            try:
                gdf_capa = gpd.read_file(ARCHIVO_KML, driver='KML', layer=capa)
                # Filtrar solo líneas (ignoramos puntos o polígonos)
                lineas = gdf_capa[gdf_capa.geometry.type.isin(['LineString', 'MultiLineString'])]
                if not lineas.empty:
                    lista_gdfs.append(lineas)
            except Exception as e:
                print(f"⚠️ Error leyendo capa '{capa}': {e}")

        if not lista_gdfs:
            print("❌ No se encontraron rutas en ninguna capa.")
            return None

        # 3. Unir todas las capas en un solo GeoDataFrame
        mapa_completo = pd.concat(lista_gdfs, ignore_index=True)
        
        # 4. Crear el Grafo Unificado
        GRAFO_GLOBAL = momepy.gdf_to_nx(mapa_completo, approach='primal')
        print(f"✅ Mapa unificado cargado. Total tramos: {len(mapa_completo)}")
        return GRAFO_GLOBAL

    except Exception as e:
        print(f"⚠️ Error CRÍTICO cargando KML: {e}")
        return None
...
# =============================================================================
# 3. MOTOR DE RUTEO HÍBRIDO (KML -> API -> RECTA)
# =============================================================================

def get_segment_route(p1, p2, G_kml):
    segment_data = {'coords': [], 'dist': 0}

    # INTENTO 1: KML UNIFICADO
    if G_kml:
        try:
            ...
            pass # No hay conexión en el mapa interno

    # INTENTO 2: API (GraphHopper)
    try:
