...
def generate_kml_conductor(route_name, points_sequence, path_coordinates):
    """
    Genera un KML para Organic Maps con la línea de la ruta y los puntos.
    """
    kml = []
    ...
    # Estilo de línea (Azul grueso para resaltar)
    kml.append('<Style id="rutaStyle">')
    kml.append('<LineStyle><color>ff0000ff</color><width>5</width></LineStyle>')
    kml.append('</Style>')

    # 1. TRAZA DEL CAMINO (TRACK)
    if path_coordinates:
        kml.append('<Placemark>')
        kml.append('<name>Camino</name>')
        kml.append('<styleUrl>#rutaStyle</styleUrl>')
        kml.append('<LineString>')
        kml.append('<coordinates>')
        # Formato KML: lon,lat,0
    ...
    for i, coords in enumerate(points_sequence):
        ...
        label = f"{i}. {lote_name}"
        if i == 0: label = "🏁 INICIO (Ingenio)"
        if i == len(points_sequence)-1: label = "🏁 FIN (Ingenio)"

        kml.append('<Placemark>')
        kml.append(f'<name>{label}</name>')
        kml.append('<Point>')
        ...
        if grp:
            ...
            if resp:
                ...
                results[key] = {
                    ...
                    "kml_data": kml_str # <--- ESTO ES LO QUE DESCARGARÁ ORGANIC MAPS
                }
