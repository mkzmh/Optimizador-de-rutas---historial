    ...
    if not use_simulation:
        try:
            ...
            if api_token == "TOKEN_NO_CONFIGURADO" or api_token == "PEGA_AQUÍ_TU_TOKEN_REAL_DE_PRAXYS":
                st.error("❌ TOKEN de Praxys no configurado en secrets.toml. Usando simulación estática.")
            ...
            st.error(f"❌ Error de conexión con Praxys para Camión {camion_id}: {e}")
            return None, None
        except Exception as e:
...
# =============================================================================
    ...
    st.info("La ubicación del camión se actualiza cada 3 segundos. Por defecto está en modo Simulación.")
    ...
    # 2. Obtener ubicaciones y Recargar la página automáticamente
    with st.empty(): # Contenedor para la actualización automática
        while True:
            # 2.1 Obtener ubicación de Camión A
            # El modo de simulación se maneja automáticamente dentro de la función
            ...
            # 2.5 Lógica de avance en la simulación (solo si estamos en simulación)
            if use_simulation:
                # Simular avance del tiempo y forzar la recarga del bucle
                time.sleep(3) 
                # Con st.empty y while True, Streamlit se encargará de recargar y actualizar el mapa.
            else:
                # En modo API real, salimos del bucle para que Streamlit use su mecanismo normal de recarga 
                # y caché para la siguiente llamada.
                break
